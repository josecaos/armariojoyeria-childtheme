	...
	<?php 
		if ( is_single() ) { ?>
			<div class="post-thumbnail">
				<?php echo do_shortcode(get_post_meta( $post->ID, '_chamo_meta_value_key', true )); ?>
			</div>
		<?php }
	?>
	<?php if ( !is_single() ) { ?>
			...
			<header class="entry-header">
				...
				<?php else : ?>
					<span class="post-date">
						<?php echo '<span class="day">'.get_the_date('d', $post->ID).'</span><span class="month">'.get_the_date('M', $post->ID).'</span><span class="year">'.get_the_date('Y', $post->ID).'</span>' ;?>
					</span>
			...
			<?php if ( is_single() ) : ?>
				...
				<div class="author-info">
					...
					<div class="author-description">
						<h2><?php esc_html__( 'About the Author:', 'chamo'); printf( '<a href="'.esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'" rel="author">%s</a>' , get_the_author()); ?></h2>
					...
					<div class="row">
						<?php
					    	...
					    	<div class="relatedthumb col-md-4">
						        ...
						        <a class="readmore button" href="<?php get_the_permalink($post->ID); ?>"><?php if(isset($chamo_opt['readmore_text']) && $chamo_opt['readmore_text']!=''){ echo esc_html($chamo_opt['readmore_text']); } else { esc_html__('Read more', 'chamo');}  ?></a>
						    </div>
