...
function checkbox_regalo( $checkout ) {
  ...
  <img src="' . get_stylesheet_directory_uri() .'/img/caja-regalo-grande.png" / >
  </div>';
  woocommerce_form_field( 'checkbox_caja_grande', array(
    'type'          => 'checkbox',
    'class'         => array('checkbox_caja_grande'),
    'label'         => __('Caja grande'),
  ));

  echo '</div>
  <div class="checkbox-envoltura"><div class="thumb-regalo">
  <img src="' . get_stylesheet_directory_uri() .'/img/caja-regalo-chico.png" / >
  </div>';
...
function agrega_datos( $cart ){
  ...
  if (isset($post_data['checkbox_caja_chica'])) {
    $extracost = 25; //
    WC()->cart->add_fee( 'Caja chica:', $extracost );
  } else if (isset($post_data['checkbox_caja_grande'])) {
    $extracost = 50;
    WC()->cart->add_fee( 'Caja grande:', $extracost );
  }
